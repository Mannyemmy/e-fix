...
{
    private function safeFrontendSection(string $key)
    {
        try {
            $section = FrontendSetting::where('key', $key)->first();
            return $section ? json_decode($section->value, true) : null;
        } catch (\Throwable $e) {
            return null;
        }
    }

    private function safeLoginRegisterSection()
    {
        return $this->safeFrontendSection('login-register-setting');
    }
    ...
    public function index(Request $request){
        ...
        foreach ($sectionKeys as $key) {
            $sectionData[$key] = $this->safeFrontendSection($key);
        }
    ...
    public function userLoginView(Request $request){
        $sectionData = $this->safeLoginRegisterSection();
